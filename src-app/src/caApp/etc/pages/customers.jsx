...
function EditCustomer() {
  ...
  return (
    <div className="h-full bg-gray-100 relative">
      ...
      <main className="p-4">
        {data?.c_id && (
          <AddCustomerForm edit={true} data={data} submitForm={submitForm} />
        )}
      </main>
    </div>
  );
      ...
      <div className="bg-white p-3 rounded-md shadow-md flex items-center justify-between">
        ...
        <Link to={to || ""} replace={!to ? true : replace}>
          <div className="flex-1">
            ...
            <p className="text-gray-500 truncate w-56">{data?.address} fkjesghfkjsg sadjkfbkjfhbds khfasdjfha skhdfb ksjdh jhk </p>
        ...
        <div className="flex flex-col gap-1">
          ...
          {data.credit !== null && data.debit !== null && (
            <p className="text-center">
          ...
          <div className="flex flex-row gap-1">
            ...
            <span className="bg-green-500 text-white px-3 py-1 rounded-md">
              <PaperAirplaneIcon
                onClick={() => {
                  handleMessage(data?.whatsapp_no, message);
                }}
                width={20}
              />
            </span>
    ...
    <div className="h-full bg-gray-100 relative">
      ...
      <main className="p-4">
        ...
        <ul className="grid grid-cols-1 gap-2">
          {data?.customers &&
            filtered_data?.map((customer, key) => {
              return (
                <CustomersTemplate
                  to={`./../customer/${customer?.c_id}`}
                  key={key}
                  data={customer}
                />
              );
...
function PendingCustomers() {
  ...
  useEffect(() => {
    API.get(`/customers-pending/${_sid}?date=${service_date}`).then(
      (response) => {
        _data_(response.data);
      }
    );
...
function CollectionCustomers() {
  ...
  useEffect(() => {
    API.get(`/customers-collection/${_sid}?date=${service_date}`).then(
      (response) => {
        _data_(response.data);
      }
    );
